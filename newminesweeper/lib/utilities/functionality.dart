...
class Auto
{
  ...
  static void fun(int i,int j)
  {
    ...
    if(gboard[i][j].minesAround==0&&gboard[i][j].revealed==false&&gboard[i][j].flagged==false)
    {
      ...
      fun(i+1,j);
      fun(i+1,j+1);
      if(i!=0&&j!=0&&gboard[i-1][j-1].minesAround!=0){
        gboard[i-1][j-1].revealed=true;
        gboard[i-1][j-1].funCallBack();
      }
      if(i!=0&&gboard[i-1][j].minesAround!=0){
        gboard[i-1][j].revealed=true;
        gboard[i-1][j].funCallBack();
      }
      if(i!=0&&j!=columns-1&&gboard[i-1][j+1].minesAround!=0){
        gboard[i-1][j+1].revealed=true;
        gboard[i-1][j+1].funCallBack();
      }
      if(j!=0&&gboard[i][j-1].minesAround!=0){
        gboard[i][j-1].revealed=true;
        gboard[i][j-1].funCallBack();
      }
      if(j!=columns-1&&gboard[i][j+1].minesAround!=0){
        gboard[i][j+1].revealed=true;
        gboard[i][j+1].funCallBack();
      }
      if(i!=rows-1&&j!=0&&gboard[i+1][j-1].minesAround!=0){
        gboard[i+1][j-1].revealed=true;
        gboard[i+1][j-1].funCallBack();
      }
      if(i!=rows-1&&gboard[i+1][j].minesAround!=0){
        gboard[i+1][j].revealed=true;
        gboard[i+1][j].funCallBack();
      }
      if(i!=rows-1&&j!=columns-1&&gboard[i+1][j+1].minesAround!=0){
        gboard[i+1][j+1].revealed=true;
        gboard[i+1][j+1].funCallBack();
      }
